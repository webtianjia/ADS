<template>
  <div>
    <el-card>
      <el-form :inline="true">
        <el-form-item>
          <el-input placeholder="订单号，多个单号逗号隔开" v-model="pageParams.order_no" style="width: 300px"></el-input>
          <el-input placeholder="提单号，多个单号逗号隔开" v-model="pageParams.bill_lading_no" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="m-t">
      ...
      <el-table
        ...
        <el-table-column label="需求概述" width="500">
          <template slot-scope="scope">
            <el-row>
        ...
        <el-table-column label="收寄人信息" width="200">
          <template slot-scope="scope">
            <div>
        ...
        <el-table-column label="状态" width="220">
          <template slot-scope="scope">
            ...
            <p v-for="item in scope.row.orderRouteList">
              <template v-if="item.route_code == '007'">
                {{item.start_time | formatDate}} {{item.route_detail}}
              </template>
              <template v-else-if="item.route_code == '008'">
                {{item.arrive_time | formatDate}} {{item.route_detail}}
              </template>
              ...
              <template v-else>
                {{item.op_time | formatDate}} {{item.route_detail}}
              </template>
            </p>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="250">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.state == 'have_order' || scope.row.state == 'booking'"
                       size="mini"
                       type="danger"
    ...
    <el-dialog title="查看订单信息" :visible.sync="detailModal">
      <div class="form-horizontal" v-if="orderDetail">
          ...
          <div class="row">
            ...
            <div class="col-sm-4">
              <div class="form-group">
                ...
                <div class="col-sm-8">
                  <div class="form-control-static">{{orderDetail.orderPriceData.create_time | formatDate}}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <template v-if="orderDetail.order.bill_lading_no != null">
          <el-card class="m-t">
        ...
        <template v-if="orderDetail.order.plate_no != null">
          <el-card class="m-t">
        ...
        <template v-if="orderDetail.order.in_house_time != null">
          <el-card class="m-t">
        ...
        <template v-if="orderDetail.order.start_time!=null||orderDetail.order.arrive_time!=null||orderDetail.order.delivery_time!=null||orderDetail.order.end_time!=null">
          <el-card class="m-t">
            <div slot="header">干线运输</div>
            <div class="row">
              <div class="col-sm-4" v-if="orderDetail.order.start_time != null">
                <div class="form-group">
                  <div class="control-label col-sm-4">起运时间</div>
                  <div class="col-sm-8">
                    <div class="form-control-static">{{orderDetail.order.start_time | formatDate}}</div>
                  </div>
                </div>
              </div>
              <div class="col-sm-4" v-if="orderDetail.order.arrive_time != null">
                <div class="form-group">
                  <div class="control-label col-sm-4">到达时间</div>
                  <div class="col-sm-8">
                    <div class="form-control-static">{{orderDetail.order.arrive_time | formatDate}}</div>
                  </div>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-sm-4" v-if="orderDetail.order.delivery_time != null">
                <div class="form-group">
                  <div class="control-label col-sm-4">派送时间</div>
                  <div class="col-sm-8">
                    <div class="form-control-static">{{orderDetail.order.delivery_time | formatDate}}</div>
                  </div>
                </div>
              </div>
              <div class="col-sm-4" v-if="orderDetail.order.end_time != null">
                <div class="form-group">
                  <div class="control-label col-sm-4">完结时间</div>
                  <div class="col-sm-8">
                    <div class="form-control-static">{{orderDetail.order.end_time | formatDate}}</div>
                  </div>
                </div>
              </div>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      selectable(row, index) {
        /*
        * 订单状态为: 【已订舱, 已下单】,且  服务模式为: 【港到门】 , 【港到港】 可选
        * */
        const orderStatus = 'booking';      //订单状态 已订舱
        const port_to_door = 'port_to_door';//服务模式 港到门
        const port_to_port = 'port_to_port';//服务模式 港到港
        if (row.state === orderStatus) {
          if(row.service_mode === port_to_door || row.service_mode === port_to_port){
            return true
          }
        }
      },
