  ...
  <div>
    ...
    <el-card class="m-t">
      <el-form el-form :inline="true" :model="demandParams">
        ...
        <el-form-item>
          <el-select class="m-l-lg" v-model="demandParams.state" placeholder="请选择需求状态">
            <el-option label="全部" value=""></el-option>
            <el-option label="新需求" value="new_demand"></el-option>
            <el-option label="已报价" value="make_offers"></el-option>
            <el-option label="已确认" value="is_confirm"></el-option>
    ...
    <el-card class="m-t">
      <el-table
        :data="demand_data"
        style="width: 100%">
        <el-table-column label="需求概述" width="700">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.state!='make_offers'&&scope.row.state!='is_confirm'&&scope.row.state!='cancel'"
                       size="mini"
                       @click="update_demand(scope.$index, scope.row)">编辑
            </el-button>
            <el-button v-if="scope.row.state!='cancel' && scope.row.state!='is_confirm'"
                       size="mini"
                       type="danger"
            ...
            <el-button v-if="scope.row.state=='cancel'"
                       size="mini"
                       @click="show_demand(scope.row)">查看
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="发布需求" :visible.sync="createModal">
      <el-form :model="createForm" :rules="rules" ref="createForm" label-width="140px">
          ...
          <el-col :span="12">
            <el-form-item label="服务模式：" prop="service_mode">
              <el-select v-model="createForm.service_mode" placeholder="请选择服务模式">
                ...
                <el-option label="港对港" value="port_to_port"></el-option>
                <el-option label="港对门" value="port_to_door"></el-option>
                <el-option label="门对港" value="door_to_port"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="提货信息：" v-if="createForm.service_mode=='door_to_door' || createForm.service_mode=='door_to_port'">
          <el-row>
            <el-col :span="24">
              <el-row>
                <el-col :span="12">
                ...
                <el-col :span="12">
                  <el-form-item label="联系电话：" prop="pick_up_phone">
    ...
    <el-dialog title="查看需求" :visible.sync="showModal">
      <el-form :model="showForm" :rules="rules" ref="showForm" label-width="140px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="起运地：" prop="provenance_str">
              <el-select
                v-model="showForm.provenance_str"
                @change="getClientAirport"
                filterable
                remote
                reserve-keyword
                placeholder="请选择起运地"
          ...
          <el-col :span="12">
            <el-form-item label="目的地：" prop="destination_str">
              <el-select
                v-model="showForm.destination_str"
                @change="getClientAirport"
                filterable
                remote
                reserve-keyword
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="货物重量(KG)：" prop="need_weight">
              <el-input v-model="showForm.need_weight" style="width: 217px"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="服务模式：" prop="service_mode">
              <el-select v-model="showForm.service_mode" placeholder="请选择服务模式">
                <el-option label="门对门" value="door_to_door"></el-option>
          ...
          <el-col :span="12">
            <el-form-item label="报关方式：" prop="declaration_type">
              <el-select v-model="showForm.declaration_type" placeholder="请选择服务模式">
                <el-option label="自主报关" value="autonomy"></el-option>
                <el-option label="委托报关" value="entrust"></el-option>
              </el-select>
            </el-form-item>
        ...
        <el-form-item label="货物信息描述：" prop="info_describe">
          <el-input type="textarea" v-model="showForm.info_describe"></el-input>
        </el-form-item>
        <el-form-item label="特殊要求备注：" prop="require_remark">
          <el-input type="textarea" v-model="showForm.require_remark"></el-input>
        </el-form-item>
        <el-form-item label="提货信息：" v-if="showForm.service_mode=='door_to_door' || showForm.service_mode=='port_to_port'">
          <el-row>
            <el-col :span="24">
              <el-row>
                <el-col :span="12">
                  <el-form-item label="联系人：" prop="pick_up_name">
                    <el-input v-model="showForm.pick_up_name"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="联系电话：" prop="pick_up_phone">
                    <el-input v-model="showForm.pick_up_phone"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24">
                  <el-form-item label="详细地址：" prop="pick_up_address">
                    <el-input type="textarea" v-model="showForm.pick_up_address"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-col>
    ...
    <el-dialog title="修改需求" :visible.sync="updateModal">
      <el-form :model="updateForm" :rules="rules" ref="updateForm" label-width="140px">
            ...
            <el-form-item label="起运地：" prop="provenance_str">
              <el-select
                ...
                <el-option
                  v-for="item in clientAirport"
                  :key="item.id"
                  :label="item.airport_name_zh+'('+item.airpor_code+')'"
                  :value="item.airport_name_zh+','+item.airpor_code">
                </el-option>
              </el-select>
            </el-form-item>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="服务模式：" prop="service_mode">
              <el-select v-model="updateForm.service_mode" placeholder="请选择服务模式" @change="select_serviceMode()">
                <el-option label="门对门" value="door_to_door"></el-option>
                <el-option label="港对港" value="port_to_port"></el-option>
                <el-option label="港对门" value="port_to_door"></el-option>
                <el-option label="门对港" value="door_to_port"></el-option>
        ...
        <el-form-item label="提货信息: " v-if="updateForm.service_mode=='door_to_door' || updateForm.service_mode=='door_to_port'">
          <el-row>
            <el-col :span="24">
              <el-row>
                <el-col :span="12">
      ...
      <div class="form-horizontal">
        ...
        <el-table v-if="scheme_quotation_data&&scheme_quotation_data.schemePriceList"
          ...
          <el-table-column label="空运价格">
            <template slot-scope="scope">
              <p v-if="scope.row.shipping_price != null">{{scope.row.shipping_price}} {{scope.row.price_currency}}</p>
              <p v-if="scope.row.min_freight != null">最低 {{scope.row.min_freight}} {{scope.row.price_currency}}</p>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules: {
          ...
          pick_up_phone: [
            {required: true, validator: this.$valid.getPhoneValidator(), trigger: 'change'}
          ],
          pick_up_address: [
    ...
    methods: {
      ...
      * */
      ...
      update_demand(index, row) {
        this.updateForm = row;
        this.updateForm.provenance_str = row.provenance + ',' + row.provenance_code;
        this.updateForm.destination_str = row.destination + ',' + row.destination_code;
        this.show_update_modal('updateForm')
      },

      select_serviceMode() {
        if (this.updateForm.service_mode == 'port_to_door' || this.updateForm.service_mode == 'port_to_port') {
          this.updateForm.pick_up_name = '';
          this.updateForm.pick_up_phone = '';
          this.updateForm.pick_up_address = null;
        }
      },
     ...
     * */
      ...
      * */
      ...
      closeAllModal() {
        this.scheme_quotation_Modal = false//方案报价弹框,
        this.create_order_Modal = false//订舱录入弹框
      },
      /*添加一行货物*/
      addRow() {
      ...
      submit_create_order(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let data = Object.assign({}, this.create_order_data);
      ...
      remoteMethod(query) {//起运目的地远程搜索
        ...
        if (query !== '') {
          var data = {"condition": query}
          this.remoteMethodLoding = true;
          API.querySite(data).then((response) => {
            this.clientAirport = response.data.list;
          })
        }
      },
      ...
      show_demand(row) {
        this.showForm = row;
        this.showForm.provenance_str = row.provenance_code + ',' + row.provenance;
        this.showForm.destination_str = row.destination_code + ',' + row.destination;
        this.show_modal("showForm");
