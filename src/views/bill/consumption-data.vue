    ...
    <el-card class="m-t">
      <el-table
        ...
        <el-table-column label="出货时间">
          <template slot-scope="scope">
            <p v-if="scope.row.shipment_time">{{scope.row.shipment_time|formatDate}}</p>
            <p v-else>暂无</p>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini"
                       @click="getDetail(scope.row.pay_id)">查看明细
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(pay_id) {
        API.getDetail({pay_id: pay_id}).then((response => {
          if (response.code == 1) {
...
//            console.log(response)
      ...
      submitSearch() {
        this.pageParams.shipment_startTime = ''
        this.pageParams.shipment_endTime = ''
        if (this.shipmentArray) {
          this.pageParams.shipment_startTime = this.shipmentArray[0]
          this.pageParams.shipment_endTime = this.shipmentArray[1]
        }
        this.getConsumptionList()
      }
